...
import { QUERY } from 'src/components/Glider/GlidersCell'
    ...
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        ...
        <tbody>
          {gliders.map((glider) => (
            <tr key={glider.id}>
              <td>{truncate(glider.id)}</td>
              <td>{truncate(glider.gliderTypeId)}</td>
              <td>{truncate(glider.callsign)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.glider({ id: glider.id })}
                    title={'Show glider ' + glider.id + ' detail'}
